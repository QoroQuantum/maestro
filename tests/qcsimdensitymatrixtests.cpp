...
struct QCSimDensityMatrixRandomCircuitsFixture {
  QCSimDensityMatrixRandomCircuitsFixture() {
    qcsimSV = MakeQCSim(Simulators::SimulationType::kStatevector,
                        nrQubitsForRandomCirc);
    qcsimDensity = MakeQCSim(Simulators::SimulationType::kDensityMatrix,
                             nrQubitsForRandomCirc);
  ...
  std::shared_ptr<Simulators::ISimulator> qcsimDensity;
...
BOOST_DATA_TEST_CASE_F(QCSimDensityMatrixRandomCircuitsFixture,
  ...
  for (size_t stateIdx = 0; stateIdx < nrStates; ++stateIdx) {
    ...
    BOOST_TEST(std::abs(pdm - psv) < kQCSimDensityTolerance,
               "Probability mismatch for outcome " << stateIdx << ": expected "
                                                << psv << ", got " << pdm);
  }

  resetRandomCirc->Execute(qcsimDensity, state);
...
BOOST_DATA_TEST_CASE_F(QCSimDensityMatrixRandomCircuitsFixture,
  ...
  auto dmCounts = qcsimDensity->SampleCountsMany(sampledQubits, shots);
