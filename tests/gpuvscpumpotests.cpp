/** Extensive GPU-vs-CPU (QCSim) cross-checks for the matrix-product-operator
 * (MPO) backend. */
#ifdef __linux__
...
std::shared_ptr<Simulators::ISimulator> MakeQCSimMPO(
...
std::shared_ptr<Simulators::ISimulator> MakeGpuMPO(
    size_t qubits = kNumQubits) {
...
// CSwap), which neither backend's MPO representation supports natively -
// both decompose them into 1- and 2-qubit gates.
...
struct QCSimVsGpuMPOFixture {
  QCSimVsGpuMPOFixture() {
    qcsimMPO = MakeQCSimMPO();
    gpuMPO = MakeGpuMPO();
...
BOOST_AUTO_TEST_SUITE(gpu_vs_cpu_mpo_tests)

BOOST_AUTO_TEST_CASE(configuration_matches) {
  auto gpuMPO = MakeGpuMPO();
  if (!gpuMPO) {
    BOOST_TEST_MESSAGE("GPU matrix-product-operator library is unavailable; "
                       "skipping");
    return;
  }
  auto qcsimMPO = MakeQCSimMPO();
  BOOST_TEST(gpuMPO->GetConfiguration("method") ==
             qcsimMPO->GetConfiguration("method"));
  BOOST_TEST(static_cast<int>(gpuMPO->GetSimulationType()) ==
             static_cast<int>(qcsimMPO->GetSimulationType()));
  CheckAllClose(gpuMPO->AllProbabilities(), qcsimMPO->AllProbabilities());
}

BOOST_AUTO_TEST_CASE(basis_state_initialization_matches) {
  auto gpuMPO = Simulators::SimulatorsFactory::CreateSimulator(
      Simulators::SimulatorType::kGpuSim,
      Simulators::SimulationType::kMatrixProductOperator);
  if (!gpuMPO) {
    BOOST_TEST_MESSAGE("GPU matrix-product-operator library is unavailable; "
  ...
  auto qcsimMPO = Simulators::SimulatorsFactory::CreateSimulator(
      Simulators::SimulatorType::kQCSim,
      Simulators::SimulationType::kMatrixProductOperator);
  ...
  for (Types::qubit_t basisState = 0; basisState < (1ULL << kNumQubits);
       ++basisState) {
    gpuMPO->InitializeToBasisState(kNumQubits, basisState);
    qcsimMPO->InitializeToBasisState(kNumQubits, basisState);

    CheckAllClose(gpuMPO->AllProbabilities(), qcsimMPO->AllProbabilities());
    for (const auto& pauli : paulis)
      BOOST_TEST(std::abs(gpuMPO->ExpectationValue(pauli) -
                          qcsimMPO->ExpectationValue(pauli)) < kTolerance,
...
BOOST_AUTO_TEST_CASE(mixture_of_basis_states_initialization_matches) {
  ...
  gpuMPO->InitializeToMixtureOfBasisStates(kNumQubits, mixture);
  qcsimMPO->InitializeToMixtureOfBasisStates(kNumQubits, mixture);

  CheckAllClose(gpuMPO->AllProbabilities(), qcsimMPO->AllProbabilities());
  BOOST_CHECK_CLOSE(gpuMPO->Probability(0), 0.5, 1e-4);
  BOOST_CHECK_CLOSE(gpuMPO->Probability(3), 0.2, 1e-4);
...
BOOST_AUTO_TEST_CASE(all_gates_and_channels_direct_application_matches) {
  ...
  ApplyAllGatesOnce(*gpuMPO);
  ApplyAllGatesOnce(*qcsimMPO);

  CheckAllClose(gpuMPO->AllProbabilities(), qcsimMPO->AllProbabilities());

  // Apply an amplitude-damping channel identically and check again.
  gpuMPO->ApplyQuantumChannel(
      {0}, Simulators::QuantumChannel::AmplitudeDamping(0.3));
  qcsimMPO->ApplyQuantumChannel(
      {0}, Simulators::QuantumChannel::AmplitudeDamping(0.3));
  CheckAllClose(gpuMPO->AllProbabilities(), qcsimMPO->AllProbabilities());

  // Unlike a pure-state (MPS/statevector) reset, MPO::ApplyReset is a
  // deterministic quantum channel (trace out the qubit, reinitialize to
  // |0><0|) rather than a stochastic single-shot measurement, so comparing
  // AllProbabilities() directly after it is meaningful here.
  gpuMPO->ApplyReset({1, 2});
  qcsimMPO->ApplyReset({1, 2});
  CheckAllClose(gpuMPO->AllProbabilities(), qcsimMPO->AllProbabilities());
}
...
BOOST_AUTO_TEST_CASE(all_gates_sampling_matches) {
  ...
  auto qcsimCounts = qcsimMPO->SampleCounts(allQubits, shots);
  auto gpuCounts = gpuMPO->SampleCounts(allQubits, shots);
...
BOOST_AUTO_TEST_CASE(all_gates_repeated_measurement_matches) {
  ...
  for (size_t trial = 0; trial < trials; ++trial) {
    if (trial > 0) {
      gpuMPO->RestoreState();
      qcsimMPO->RestoreState();
    }
    ++gpuCounts[gpuMPO->MeasureMany(allQubits)];
    ++qcsimCounts[qcsimMPO->MeasureMany(allQubits)];
  }
...
BOOST_DATA_TEST_CASE_F(QCSimVsGpuMPOFixture, RandomCircuitsMatch,
  ...
  for (const auto& pauli : paulis)
    BOOST_TEST(std::abs(gpuMPO->ExpectationValue(pauli) -
                        qcsimMPO->ExpectationValue(pauli)) < 1e-5,
               "Pauli " << pauli << " mismatch at " << nrGates << " gates");

  resetCirc->Execute(gpuMPO, state);
  resetCirc->Execute(qcsimMPO, state);
  state.Reset();
}

BOOST_DATA_TEST_CASE_F(QCSimVsGpuMPOFixture, SamplingMatches,
...
BOOST_AUTO_TEST_CASE(measurement_collapse_marginals_match) {
  auto gpuMPO = MakeGpuMPO(2);
  if (!gpuMPO) {
    BOOST_TEST_MESSAGE("GPU matrix-product-operator library is unavailable; "
                       "skipping");
    return;
  }
  auto qcsimMPO = MakeQCSimMPO(2);
  ...
  for (int i = 0; i < trials; ++i) {
    ...
    gpuMPO->ApplyH(0);
    gpuMPO->ApplyCX(0, 1);
    const auto gpuMeasured = gpuMPO->MeasureMany({0, 1});
    ...
    qcsimMPO->ApplyH(0);
    qcsimMPO->ApplyCX(0, 1);
    const auto qcsimMeasured = qcsimMPO->MeasureMany({0, 1});
