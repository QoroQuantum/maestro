...
{
	TensorsTestFixture()
		: g(std::random_device{}()), boolDist(0.5)
	{
		// setup
		...
		qc->Initialize();
		qcForest = Simulators::SimulatorsFactory::CreateSimulator(Simulators::SimulatorType::kQCSim, Simulators::SimulationType::kStatevector);
		...
		//const auto tensorContractor = std::make_shared<TensorNetworks::StochasticContractor>();
		//const auto tensorContractor = std::make_shared<TensorNetworks::VerticalContractor>();
	    const auto tensorContractor = std::make_shared<TensorNetworks::ForestContractor>();
		//const auto tensorContractor = std::make_shared<TensorNetworks::DumbContractor>();
		//const auto tensorContractor = std::make_shared<TensorNetworks::LookaheadContractor>();
		tensorNetwork->SetContractor(tensorContractor);
		tensorNetworkOneQubit->SetContractor(tensorContractor);

		tensorNetworkForest = std::make_shared<TensorNetworks::TensorNetwork>(nrQubitsForest);
		const auto tensorContractorForest = std::make_shared<TensorNetworks::ForestContractor>();
		...
		qcsimMPS->SetMultithreading(false); // doesn't implement multithreading yet, anyway
		qcsimMPS->Initialize();
	}
	...
	void GenerateCircuit(int nrGates, bool avoidParamGates = false)
	{
		...
		for (int gateNr = 0; gateNr < nrGates; ++gateNr, ++gateGenIter)
		{
			...
			if (avoidParamGates && (gateType == Circuits::QuantumGateType::kCUGateType || gateType == Circuits::QuantumGateType::kCPGateType || gateType == Circuits::QuantumGateType::kPhaseGateType
				|| gateType == Circuits::QuantumGateType::kRxGateType || gateType == Circuits::QuantumGateType::kRyGateType || gateType == Circuits::QuantumGateType::kRzGateType
				|| gateType == Circuits::QuantumGateType::kUGateType
				|| gateType == Circuits::QuantumGateType::kCRxGateType || gateType == Circuits::QuantumGateType::kCRyGateType || gateType == Circuits::QuantumGateType::kCRzGateType))
			{
				--gateNr;
				continue;
			}
	...
	void GenerateCircuits(int nrGates, bool genThreeQubitGates = false, bool avoidParamGates = false)
	{
		auto dblGen = bdata::random(-2. * M_PI, 2. * M_PI);
		auto dblGenIter = dblGen.begin();

		auto gateGen = bdata::random(0, static_cast<int>(genThreeQubitGates ? Circuits::QuantumGateType::kCCXGateType : Circuits::QuantumGateType::kCUGateType));
		auto gateGenIter = gateGen.begin();
		...
		for (int gateNr = 0; gateNr < nrGates; ++gateNr, ++gateGenIter)
		{
			...
			// first, pick randomly two qubits (depending on the randomly chosen gate type, not all of them will be used)
			std::shuffle(qubits.begin(), qubits.end(), g);
			...
			const auto theGate = Circuits::CircuitFactory<>::CreateGate(gateType, q1, q2, q3, param1, param2, param3, param4);

			randomCirc->AddOperation(theGate);

			// when three qubit gates are requested, the simulation won't happen directly with the tensor network, but through a simulator, which does transpiling for the three qubit gates
			// this is only used for tests with the tensor network directly
			if (!genThreeQubitGates)
				AddQcSimGate(gateType, q1, q2, param1, param2, param3, param4);
		}
	}

	void GenerateForestCircuits(int nrGates, bool genOnlyClifforGates = false, bool avoidParamGates = false)
	{
		std::bernoulli_distribution newForestDist(0.1); // produce a new root with a low probability
		
		auto dblGen = bdata::random(-2. * M_PI, 2. * M_PI);
		auto dblGenIter = dblGen.begin();

		auto gateGen = bdata::random(0, static_cast<int>(Circuits::QuantumGateType::kCUGateType));
		//auto singleQubitGateGen = bdata::random(0, static_cast<int>(Circuits::QuantumGateType::kUGateType));
		auto gateGenIter = gateGen.begin();
		...
		for (int gateNr = 0; gateNr < nrGates; ++gateNr, ++gateGenIter)
		{
			...
			{
				const auto dummyGate = Circuits::CircuitFactory<>::CreateGate(gateType, 0, 1, 0, param1, param2, param3, param4);
			...
			if (static_cast<int>(gateType) <= static_cast<int>(Circuits::QuantumGateType::kUGateType))
				q1 = qubitDist(g);
			else
			{
			...
			const auto theGate = Circuits::CircuitFactory<>::CreateGate(gateType, q1, q2, 0, param1, param2, param3, param4);
			randomCirc->AddOperation(theGate);
	...
	void AddQcSimGate(Circuits::QuantumGateType gateType, Types::qubit_t q1, Types::qubit_t q2, double param1, double param2, double param3, double param4)
	{
		switch (gateType)
		{
			case Circuits::QuantumGateType::kPhaseGateType: 
			{
				...
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(pgate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kXGateType:
			{
				QC::Gates::PauliXGate<> xgate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(xgate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kYGateType:
			{
				QC::Gates::PauliYGate<> ygate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(ygate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kZGateType:
			{
				QC::Gates::PauliZGate<> zgate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(zgate.getRawOperatorMatrix(), q1));
			...
			case Circuits::QuantumGateType::kHadamardGateType:
			{
				...
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(h.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kSGateType:
			{
				QC::Gates::SGate<> sgate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(sgate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kSdgGateType:
			{
				QC::Gates::SDGGate<> sdggate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(sdggate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kTGateType:
			{
				QC::Gates::TGate<> tgate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(tgate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kTdgGateType:
			{
				QC::Gates::TDGGate<> tdggate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(tdggate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kSxGateType:
			{
				QC::Gates::SquareRootNOTGate<> sxgate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(sxgate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kSxDagGateType:
			{
				QC::Gates::SquareRootNOTDagGate<> sxdaggate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(sxdaggate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kKGateType:
			{
				QC::Gates::HyGate<> k;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(k.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kRxGateType:
			{
				QC::Gates::RxGate<> rxgate(param1);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(rxgate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kRyGateType:
			{
				QC::Gates::RyGate<> rygate(param1);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(rygate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kRzGateType:
			{
				QC::Gates::RzGate<> rzgate(param1);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(rzgate.getRawOperatorMatrix(), q1));
			}
				break;
			case Circuits::QuantumGateType::kUGateType:
			{
				QC::Gates::UGate<> ugate(param1, param2, param3, param4);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(ugate.getRawOperatorMatrix(), q1));
			}
			break;
			case Circuits::QuantumGateType::kSwapGateType:
			{
				QC::Gates::SwapGate<> swapgate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(swapgate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCXGateType:
			{
				QC::Gates::CNOTGate<> cxgate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(cxgate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCYGateType:
			{
				QC::Gates::ControlledYGate<> cygate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(cygate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCZGateType:
			{
				QC::Gates::ControlledZGate<> czgate;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(czgate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCPGateType:
			{
				QC::Gates::ControlledPhaseShiftGate<> cpgate(param1);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(cpgate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCRxGateType:
			{
				QC::Gates::ControlledRxGate<> crxgate(param1);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(crxgate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCRyGateType:
			{
				QC::Gates::ControlledRyGate<> crygate(param1);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(crygate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCRzGateType:
			{
				QC::Gates::ControlledRzGate<> crzgate(param1);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(crzgate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCHGateType:
			{
				QC::Gates::ControlledHadamardGate<> ch;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(ch.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCSxGateType:
			{
				QC::Gates::ControlledSquareRootNOTGate<> csx;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(csx.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCSxDagGateType:
			{
				QC::Gates::ControlledSquareRootNOTDagGate<> csxdag;
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(csxdag.getRawOperatorMatrix(), q1, q2));
			}
				break;
			case Circuits::QuantumGateType::kCUGateType:
			{
				QC::Gates::ControlledUGate<> cugate(param1, param2, param3, param4);
				randomQcSimCirc.emplace_back(std::make_shared<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>(cugate.getRawOperatorMatrix(), q1, q2));
			}
				break;
			default:
	...
	// TODO: To actually be able to test the difference in speed, add paralelization to the speed test for clifford and forest tensor network simulator tests!!!!
	const unsigned int nrQubitsForest = 12; // for 16 - sometimes even 15 - the forest tensor network simulator beats the statevector one even with no paralelization in tensor networks but with paralelization in statevector!!!!
	// for no parallelization, the forest tensor network simulator is faster even for 13 qubits
	const unsigned int nrShots = 5000;

	std::shared_ptr<Circuits::Circuit<>> randomCirc;
	std::vector<std::shared_ptr<QC::Gates::AppliedGate<TensorNetworks::TensorNode::MatrixClass>>> randomQcSimCirc;
...
BOOST_DATA_TEST_CASE_F(TensorsTestFixture, TensorsSimpleRandomCircsTest, bdata::xrange(1, 20), nrGates)
{
	...
	for (int t = 0; t < 30; ++t)
	{
		...
		{
			const QC::Gates::QuantumGateWithOp<TensorNetworks::TensorNode::MatrixClass>& gate = *randomQcSimCirc[g];
			tensorNetwork->AddGate(gate, randomQcSimCirc[g]->getQubit1(), randomQcSimCirc[g]->getQubit2());
		}
		...
		for (size_t q = 0; q < nrQubits; ++q)
		{
			...
			maxTensorRank = std::max(maxTensorRank, tensorNetwork->GetContractor()->GetMaxTensorRank());
...
BOOST_DATA_TEST_CASE_F(TensorsTestFixture, TensorsSimpleRandomCircsTestSimulator, bdata::xrange(1, 20), nrGates)
{
	const size_t nrStates = 1ULL << nrQubits;
	Circuits::OperationState stateDummy(nrQubits);
...
BOOST_DATA_TEST_CASE_F(TensorsTestFixture, TensorsSimpleRandomCircsMeasTest, bdata::xrange(1, 15), nrGates)
{
	std::vector<std::pair<Types::qubit_t, size_t>> measQubits(nrQubits);
	for (size_t q = 0; q < nrQubits; ++q)
		measQubits[q] = std::make_pair(q, q);
	...
	for (int t = 0; t < 3; ++t)
	{
		...
		for (size_t q = 0; q < nrQubits; ++q)
		{
			const double prob1 = static_cast<double>(measResultsQcSim[q]) / static_cast<double>(nrShots);
			const double prob2 = static_cast<double>(measResultsTensorNetwork[q]) / static_cast<double>(nrShots);
...
BOOST_DATA_TEST_CASE_F(TensorsTestFixture, TensorsSimpleRandomForestCircsTest, bdata::xrange(1, 30), nrGates)
{
	...
	for (int t = 0; t < 3; ++t)
	{
		...
		{
			const QC::Gates::QuantumGateWithOp<TensorNetworks::TensorNode::MatrixClass>& gate = *randomQcSimCirc[g];
			tensorNetworkForest->AddGate(gate, randomQcSimCirc[g]->getQubit1(), randomQcSimCirc[g]->getQubit2());
		}
		...
		for (size_t q = 0; q < nrQubitsForest; ++q)
		{
			...
			maxTensorRank = std::max(maxTensorRank, tensorNetworkForest->GetContractor()->GetMaxTensorRank());
			...
			for (size_t state = 0; state < nrStates; ++state)
				if ((state & qubitMask) == 0)
					prob2 += qcForest->Probability(state);
...
BOOST_DATA_TEST_CASE_F(TensorsTestFixture, TensorsSimpleRandomForestCircsSimulatorTest, bdata::xrange(1, 30), nrGates)
{
	const size_t nrStates = 1ULL << nrQubitsForest;
	Circuits::OperationState stateDummy(nrQubitsForest);
...
/*
BOOST_DATA_TEST_CASE_F(TensorsTestFixture, TensorsRandomCliffordForestCircsSimulatorTest, bdata::xrange(1, 30), nrGates)
{
	...
	for (int t = 0; t < 3; ++t)
	{
		BOOST_TEST_MESSAGE("Generating circuit with " << nrGates << " gates for test #" << t);
		GenerateForestCircuits(nrGates, true);
		...
		for (size_t i = 0; i < nrQubitsForest; ++i)
			randomCirc->AddOperation(Circuits::CircuitFactory<>::CreateMeasurement({ {i, i} }));
		...
		BOOST_TEST_MESSAGE("\nExecution in qcsim statevector took: " << dif / 1000. << " seconds!");
		...
		BOOST_TEST_MESSAGE("\nExecution in qcsim tensor network took: " << dif / 1000. << " seconds!");
		...
		dif = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
		BOOST_TEST_MESSAGE("\nExecution in qcsim MPS took: " << dif / 1000. << " seconds!");
		...
		dif = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
		BOOST_TEST_MESSAGE("\nExecution in qcsim stabilizer took: " << dif / 1000. << " seconds!");
