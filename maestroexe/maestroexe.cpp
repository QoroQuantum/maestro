...
int main(int argc, char** argv)
{
	try {
		...
		desc.add_options()
			("help,h", "Print a help description")
			("version,v", "Print version information")
			("nrqubits,n", boost::program_options::value<int>(), "Specify the number of qubits")
		...
		if (simulatorType != 2 && simulatorType != 3)
		{
			...
			if (!stype.empty())
			{
				...
				if (stype == "statevector" || stype == "sv")
					...
					simulationType = 2;
				else if (stype == "tensor" || stype == "tensor_network" || stype == "tn")
					simulationType = 3;
				else
					simulationType = 1000;
			}
		...
		if (simulatorType < 2) // qcsim or aer
		{
			if (simulationType < 4)
				...
				simulator.RemoveAllOptimizationSimulatorsAndAdd(static_cast<int>(simulatorType),
					0);
		...
		else if (simulatorType < 4) // composite, ignore exec type and set statevector
		{
			simulator.RemoveAllOptimizationSimulatorsAndAdd(static_cast<int>(simulatorType), 0);
		}
