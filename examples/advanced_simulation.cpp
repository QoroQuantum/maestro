 ...
 * @brief Advanced usage example of the Maestro library using the C interface for manual control.
 *
...
int main() {
    ...
    // Simulators::SimulatorType::Statevector is typically 0 (check Simulators/Simulator.h or documentation).
    ...
    // We will create a Bell state: |00> -> H(0) -> |+0> -> CX(0,1) -> (|00> + |11>) / sqrt(2)
    ...
    // 4. Measure
    // MeasureNoCollapse returns the outcome without collapsing the state vector (useful for debugging/inspection)
    // Note: This might not be supported by all backends.
