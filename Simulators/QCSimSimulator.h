...
class QCSimSimulator : public QCSimState {
 ...
 public:
  ...
  std::unique_ptr<ISimulator> Clone() override {
    ...
    // Clone() does not copy rng (each clone gets a fresh random_device seed by
    // default). When a user seed is set, derive a deterministic, distinct seed
    // per clone so any rng-driven sampling stays reproducible (and decorrelated
    // across clones) even on multi-threaded paths. The reproducible terminal
    // sampling paths (statevector / path-integral) currently run
    // single-threaded, so this is primarily defensive. splitmix64 mixing
    // decorrelates the resulting mt19937_64 streams.
    if (userSeed_) {
      uint64_t z = *userSeed_ + 0x9E3779B97F4A7C15ULL * (++cloneCounter_);
      z = (z ^ (z >> 30)) * 0xBF58476D1CE4E5B9ULL;
      z = (z ^ (z >> 27)) * 0x94D049BB133111EBULL;
      z = z ^ (z >> 31);
      cloned->userSeed_ = z;
      cloned->cloneCounter_ = 0;
      cloned->rng.seed(z);
    }
