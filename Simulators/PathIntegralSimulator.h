...
class PathIntegralSimulator {
 public:
  ...
  size_t GetMaxDoublingsForBackwardPaths() const {
    return simulator.GetMaxDoublingsForBackwardPaths();
  }
  ...
  std::complex<double> Amplitude(const std::vector<bool>& startState,
                                 const std::vector<bool>& endState) {
    return simulator.Propagate(startState, endState);
  }

  std::optional<QC::Gates::AppliedGate<>> ConvertGate(
    ...
    switch (gate->GetGateType()) {
      ...
      case Circuits::QuantumGateType::kPhaseGateType: {
        ...
        return QC::Gates::AppliedGate<>(pgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      }
      case Circuits::QuantumGateType::kXGateType:
        return QC::Gates::AppliedGate<>(xgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kYGateType:
        return QC::Gates::AppliedGate<>(ygate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kZGateType:
        return QC::Gates::AppliedGate<>(zgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kHadamardGateType:
        return QC::Gates::AppliedGate<>(h.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kSGateType:
        return QC::Gates::AppliedGate<>(sgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kSdgGateType:
        return QC::Gates::AppliedGate<>(sdggate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kTGateType:
        return QC::Gates::AppliedGate<>(tgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kTdgGateType:
        return QC::Gates::AppliedGate<>(tdggate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kSxGateType:
        return QC::Gates::AppliedGate<>(sxgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kSxDagGateType:
        return QC::Gates::AppliedGate<>(sxdaggate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kKGateType:
        return QC::Gates::AppliedGate<>(k.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kRxGateType: {
        ...
        return QC::Gates::AppliedGate<>(rxgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      }
      case Circuits::QuantumGateType::kRyGateType: {
        ...
        return QC::Gates::AppliedGate<>(rygate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      }
      case Circuits::QuantumGateType::kRzGateType: {
        ...
        return QC::Gates::AppliedGate<>(rzgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      }
      case Circuits::QuantumGateType::kUGateType: {
        const auto g = std::static_pointer_cast<Circuits::UGate<>>(gate);
        ugate.SetParams(g->GetTheta(), g->GetPhi(), g->GetLambda(),
                        g->GetGamma());
        return QC::Gates::AppliedGate<>(ugate.getRawOperatorMatrix(),
                                        gate->GetQubit(0));
      }

      // two qubit gates
      case Circuits::QuantumGateType::kSwapGateType:
        return QC::Gates::AppliedGate<>(swapgate.getRawOperatorMatrix(),
                                        gate->GetQubit(0), gate->GetQubit(1));
      case Circuits::QuantumGateType::kCXGateType:
        return QC::Gates::AppliedGate<>(cxgate.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      case Circuits::QuantumGateType::kCYGateType:
        return QC::Gates::AppliedGate<>(cygate.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      case Circuits::QuantumGateType::kCZGateType:
        return QC::Gates::AppliedGate<>(czgate.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      case Circuits::QuantumGateType::kCPGateType: {
        ...
        return QC::Gates::AppliedGate<>(cpgate.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      }
      case Circuits::QuantumGateType::kCRxGateType: {
        ...
        return QC::Gates::AppliedGate<>(crxgate.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      }
      case Circuits::QuantumGateType::kCRyGateType: {
        ...
        return QC::Gates::AppliedGate<>(crygate.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      }
      case Circuits::QuantumGateType::kCRzGateType: {
        ...
        return QC::Gates::AppliedGate<>(crzgate.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      }
      case Circuits::QuantumGateType::kCHGateType:
        return QC::Gates::AppliedGate<>(ch.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      case Circuits::QuantumGateType::kCSxGateType:
        return QC::Gates::AppliedGate<>(csx.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      case Circuits::QuantumGateType::kCSxDagGateType:
        return QC::Gates::AppliedGate<>(csxdag.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      case Circuits::QuantumGateType::kCUGateType: {
        const auto g = std::static_pointer_cast<Circuits::CUGate<>>(gate);
        cugate.SetParams(g->GetTheta(), g->GetPhi(), g->GetLambda(),
                         g->GetGamma());
        return QC::Gates::AppliedGate<>(cugate.getRawOperatorMatrix(),
                                        gate->GetQubit(1), gate->GetQubit(0));
      }

      // three qubit gates
      case Circuits::QuantumGateType::kCCXGateType:
        return QC::Gates::AppliedGate<>(ccxgate.getRawOperatorMatrix(),
                                        gate->GetQubit(2), gate->GetQubit(1),
                                        gate->GetQubit(0));
      case Circuits::QuantumGateType::kCSwapGateType:
        return QC::Gates::AppliedGate<>(cswapgate.getRawOperatorMatrix(),
                                        gate->GetQubit(2), gate->GetQubit(1),
                                        gate->GetQubit(0));
  ...
  std::vector<QC::Gates::AppliedGate<>> ConvertCircuit(
    ...
    for (const auto& op : operations) {
      if (!op || op->GetType() != Circuits::OperationType::kGate) continue;
  ...
  static size_t GetBranchingForQcsimCircuit(
  ...
  size_t GetBranchingForMaestroCircuit(
      const std::shared_ptr<Circuits::Circuit<>>& circuit) {
  ...
  std::complex<double> AmplitudeForOutcome(size_t outcome) {
    ...
    QC::PathIntegral::FastVectorBool state(amplitudes.begin()->first.size());
    for (size_t q = 0; q < state.size(); ++q) state.set(q, (outcome >> q) & 1);
  ...
  bool MeasureQubit(size_t qubit) { return simulator.MeasureQubit(qubit); }
  ...
  double ExpectationValue(const std::string& pauliStringOrig) {
    ...
    for (size_t i = 0; i < pauliStringOrig.size(); ++i) {
      ...
      switch (c) {
        ...
        case 'x':
          PropagateStep(
              QC::Gates::AppliedGate<>(xgate.getRawOperatorMatrix(), i),
              amplitudes);
          break;
        case 'Y':
          [[fallthrough]];
        case 'y':
          PropagateStep(
              QC::Gates::AppliedGate<>(ygate.getRawOperatorMatrix(), i),
              amplitudes);
          break;
        case 'Z':
          [[fallthrough]];
        case 'z':
          PropagateStep(
              QC::Gates::AppliedGate<>(zgate.getRawOperatorMatrix(), i),
              amplitudes);
          break;
        default:
          break;
    ...
    for (const auto& [state, amplitude] : amplitudes) {
      if (currentAmplitudes.find(state) == currentAmplitudes.end()) continue;
