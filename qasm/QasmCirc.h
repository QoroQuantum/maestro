	...
	public:
		...
		std::shared_ptr<Circuits::Circuit<Time>> ParseAndTranslate(std::string& qasmInput)
		{
			...
			try {
				...
				if (boost::spirit::qi::phrase_parse(it, qasmInput.end(), grammar, qasm::ascii::space, program))
				{
					if (it == qasmInput.end())
					{
						return program.ToCircuit<Time>(grammar.opaqueGates, grammar.definedGates);
					}
					else
					{
						error = true;
						errorMessage = "Error: Unparsed input remaining: '" + std::string(it, qasmInput.end()) + "'";
					}
				}
				else
				{
					error = true;
					errorMessage = "Error: Parsing failed, unparsed input remaining: '" + std::string(it, qasmInput.end()) + "'";
				}
			}
