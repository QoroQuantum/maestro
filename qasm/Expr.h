...
namespace qasm {
	...
	public:
		...
		virtual double Eval(const std::unordered_map<std::string, double>& variables) const { return 0; }
	...
	template<typename Expr> static AbstractSyntaxTreePtr Clone(Expr const& t) { return std::make_shared<Expr>(t); }

	// for expressions (to be evaluated, typically those are values for parameters for gates)
	...
	public:
		...
		Constant& operator=(int value) { this->value = value; return *this; }
		Constant& operator=(double value) { this->value = value; return *this; }

		double Eval() const override { return value; }
		double Eval(const std::unordered_map<std::string, double>& variables) const override { return value; }
	...
	struct MakeConstantExpression {
		template<typename> struct result { typedef Constant type; };
	...
	public:
		...
		Variable& operator=(int value) { this->value = value; return *this; }
	...
	struct MakeVariableExpression {
		template<typename> struct result { typedef Variable type; };
	...
	public:
		...
		double Eval() const override
		{
			...
			case '*': return left->Eval() * right->Eval();
			case '/': return left->Eval() / right->Eval();
			case '^': return pow(left->Eval(), right->Eval());
		...
		double Eval(const std::unordered_map<std::string, double>& variables) const override
		{
			switch (op)
			{
			case '+': return left->Eval(variables) + right->Eval(variables);
			case '-': return left->Eval(variables) - right->Eval(variables);
			case '*': return left->Eval(variables) * right->Eval(variables);
			case '/': return left->Eval(variables) / right->Eval(variables);
			case '^': return pow(left->Eval(variables), right->Eval(variables));
			default: throw std::invalid_argument("Unknown binary operator");
			}
	...
	struct MakeBinaryExpression {
		template<typename, typename, typename> struct result { typedef BinaryOperator type; };
	...
	public:
		...
		double Eval(const std::unordered_map<std::string, double>& variables) const override
		{
			switch (op)
			{
			case '+': return right->Eval(variables);
			case '-': return -right->Eval(variables);
			default: throw std::invalid_argument("Unknown unary operator");
			}
			return 0;
	...
	struct MakeUnaryExpression {
		template<typename, typename> struct result { typedef UnaryOperator type; };
	...
	public:
		...
		double Eval() const override
		{
			if (func == "sin") return sin(param->Eval());
			else if (func == "cos") return cos(param->Eval());
			else if (func == "tan") return tan(param->Eval());
			else if (func == "exp") return exp(param->Eval());
			else if (func == "ln") return log(param->Eval());
			else if (func == "sqrt") return sqrt(param->Eval());
		...
		double Eval(const std::unordered_map<std::string, double>& variables) const override
		{
			if (func == "sin") return sin(param->Eval(variables));
			else if (func == "cos") return cos(param->Eval(variables));
			else if (func == "tan") return tan(param->Eval(variables));
			else if (func == "exp") return exp(param->Eval(variables));
			else if (func == "ln") return log(param->Eval(variables));
			else if (func == "sqrt") return sqrt(param->Eval(variables));
	...
	struct MakeFunctionExpression {
		template<typename, typename, typename> struct result { typedef Function type; };
