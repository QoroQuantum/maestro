...
namespace qasm {
	...
	public:
		...
		operator std::string() const { return declType + " " + id + "[" + std::to_string(index) + "]"; }
	...
	struct MakeIndexedIdExpression {
		template<typename, typename> struct result { typedef IndexedId type; };

		template<typename ID, typename IND> IndexedId operator()(const ID& id, IND index) const
		{
	...
	{
		struct result { typedef IndexedId type; };
	...
	struct AddGateDeclExpr : public AbstractSyntaxTree
	{
		...
		QoperationStatement operator()(const boost::fusion::vector<GateDeclType, std::vector<GateDeclOpType>>& gateDecl, std::unordered_map<std::string, StatementType>& definedGates) const
		{
			...
			if (args.empty())
				throw std::invalid_argument("Gate declaration must have at least one qubit argument: " + gateName);
			else if (definedGates.find(gateName) != definedGates.end()) // for now do not allow redefinition, the biggest problem is that defined gates can be used inside other defined gates, otherwise redefinition would be simple to handle
			...
			const std::vector<GateDeclOpType>& declOps = boost::fusion::at_c<1>(gateDecl);
