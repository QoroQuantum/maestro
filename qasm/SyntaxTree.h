...
namespace qasm {
	...
	{
		struct result { typedef QoperationStatement type; };
		...
		QoperationStatement operator()(const UopType& uop, const std::unordered_map<std::string, IndexedId>& qreg_map, const std::unordered_map<std::string, StatementType>& opaqueGates, const std::unordered_map<std::string, StatementType>& definedGates, const std::unordered_map<std::string, double>& variables = {}) const
		{
			...
			else if (std::holds_alternative<CXGateCallType>(uop))
			{
				...
				else if (qubits1.size() == 1 && qubits2.size() > 1)
				{
					...
					{
						if (q2 == qubits1[0])
							throw std::invalid_argument("Control and target qubits cannot be the same for CX gate");
						stmt.qubits.push_back(qubits1[0]);
				...
				else throw std::invalid_argument("Mismatched qubits sizes for CX gate arguments");
			}
			else if (std::holds_alternative<GatecallType>(uop))
			{
				...
				if (std::holds_alternative<SimpleGatecallType>(gateCall))
				{
					...
					std::transform(gateNameLower.begin(), gateNameLower.end(), gateNameLower.begin(), ::tolower);
					...
					if (it == definedGates.end())
					{
						if (!IsSuppportedGate(gateNameLower))
							throw std::invalid_argument("Unsupported gate without parameters: " + gateName);
						...
						if (args.size() != expectedNrQubits)
							throw std::invalid_argument("Gate " + gateName + " requires exactly " + std::to_string(expectedNrQubits) + " qubits");
					}
					else
					{
						// defined gate, check if the number of qubits match
						const StatementType& definedGateStmt = it->second;
						const int expectedNrQubits = static_cast<int>(definedGateStmt.qubitsDecl.size());
						if (args.size() != expectedNrQubits)
							throw std::invalid_argument("Defined gate " + gateName + " requires exactly " + std::to_string(expectedNrQubits) + " qubits");
						if (definedGateStmt.parameters.size() != 0)
							throw std::invalid_argument("Defined gate " + gateName + " requires parameters, but none were provided");

						stmt.comment = gateName;

						// copy the gate definition here, as it might be redefined later (actually not yet supported)
					...
					for (int i = 0; i < static_cast<int>(args.size()); ++i)
					{
						...
						if (qubits.size() != 1)
						{
							if (qubitsCounter == 1)
								...
								throw std::invalid_argument("Mismatched qubits sizes for gate arguments");
						}
				...
				else if (std::holds_alternative<ExpGatecallType>(gateCall))
				{
					...
					if (it == definedGates.end())
					{
						...
						{
							if (params.empty())
								throw std::invalid_argument("Gate " + gateName + " requires parameters");
							else if (params.size() == 1)
							{
								if (!IsSuppportedOneParamGate(gateNameLower))
									throw std::invalid_argument("This gate does not allow one parameter: " + gateName);
							}
							else if (params.size() > 1)
							{
								if (!IsSuppportedMultipleParamsGate(gateNameLower))
									throw std::invalid_argument("This gate does not allow multiple parameters: " + gateName);

								if (params.size() > 4)
									throw std::invalid_argument("Too many parameters for gate: " + gateName);
							}
						...
						else if (gateNameLower == "u1")
						{
							...
							if (stmt.parameters.size() != 1)
								throw std::invalid_argument("Gate u1 requires exactly one parameter");
							...
							if (args.size() != 1)
								throw std::invalid_argument("Gate " + gateName + " requires exactly 1 qubit");
						}
						else if (gateNameLower == "u2")
						{
							stmt.gateType = Circuits::QuantumGateType::kUGateType;
						...
						else if (gateNameLower == "cu1")
						{
							...
							if (stmt.parameters.size() != 1)
								throw std::invalid_argument("Gate cu1 requires exactly one parameter");
							...
							if (args.size() != 2)
								throw std::invalid_argument("Gate " + gateName + " requires exactly 2 qubits");
						}
						else if (gateNameLower == "cu2")
						{
							stmt.gateType = Circuits::QuantumGateType::kCUGateType;
					...
					else
					{
						...
						if (definedGateStmt.paramsDecl.size() != stmt.parameters.size())
							throw std::invalid_argument("Defined gate " + gateName + " requires a different number (" + std::to_string(definedGateStmt.paramsDecl.size()) + ") of parameters than " + std::to_string(stmt.parameters.size()));
		...
		static std::vector<int> ParseQubits(const ArgumentType& arg, const std::unordered_map<std::string, IndexedId>& qreg_map)
		{
			...
			// there are two possibilities here, either it's an indexed id or a simple id
		...
		{
			return allowedMultipleParamsGates.find(gateName) != allowedMultipleParamsGates.end();
		}
		...
		static bool IsSuppportedGate(const std::string& gateName)
		{
			return allowedNoParamGates.find(gateName) != allowedNoParamGates.end() ||
				...
				allowedMultipleParamsGates.find(gateName) != allowedMultipleParamsGates.end();
		}

		Circuits::QuantumGateType GetGateType(const std::string& gateN) const
		{
			std::string gateName = gateN;
			std::transform(gateName.begin(), gateName.end(), gateName.begin(), ::tolower);
			...
			return Circuits::QuantumGateType::kNone; // this will be returnd also for 'id'
		}
		...
		static int GateNrQubits(Circuits::QuantumGateType gateType)
		{
			...
			if (gateT < static_cast<int>(Circuits::QuantumGateType::kSwapGateType))
				return 1;
			else if (gateT < static_cast<int>(Circuits::QuantumGateType::kCSwapGateType))
				return 2;
	...
	private:
		static inline std::unordered_set<std::string> allowedNoParamGates = { "x", "y", "z", "h", "s", "sdg","sdag", "t", "tdg", "tdag", "sx", "sxdg", "sxdag", "k", "swap", "cx", "cy", "cz", "ch", "csx", "csxdg", "csxdag", "cswap", "ccx", "id" };
		static inline std::unordered_set<std::string> allowedOneParamGates = { "p", "rx", "ry", "rz", "cp", "crx", "cry", "crz", "u1", "cu1" };
		static inline std::unordered_set<std::string> allowedMultipleParamsGates = { "u", "u3", "cu1", "cu3" }; // max 4
	};
	...
	struct Program
	{
		...
		template<typename Time = Types::time_type> static void AddToCircuit(const std::shared_ptr<Circuits::Circuit<Time>>& circuit, const StatementType& stmt, std::unordered_map<std::string, StatementType>& opaqueGates, std::unordered_map<std::string, StatementType>& definedGates)
		{
			switch (stmt.opType)
			{
			case QoperationStatement::OperationType::Measurement:
			{
				if (stmt.qubits.size() != stmt.cbits.size())
					throw std::invalid_argument("Measurement operation: number of qubits and classical bits do not match.");

				std::vector<std::pair<Types::qubit_t, size_t>> qs;
				for (size_t i = 0; i < stmt.qubits.size(); ++i)
					qs.push_back({ static_cast<Types::qubit_t>(stmt.qubits[i]), static_cast<size_t>(stmt.cbits[i]) });
			...
			case QoperationStatement::OperationType::Uop:
			{
				if (stmt.gateType != Circuits::QuantumGateType::kNone)
				{
					...
					if (stmt.qubits.size() % nrQubits != 0)
						throw std::invalid_argument("Uop operation: number of qubits does not match the gate requirements.");

					for (int pos = 0; pos < static_cast<int>(stmt.qubits.size()); pos += nrQubits)
					{
						Types::qubits_vector gateQubits;
						for (int q = 0; q < nrQubits; ++q)
							gateQubits.push_back(static_cast<Types::qubit_t>(stmt.qubits[pos + q]));

						auto gateOp = Circuits::CircuitFactory<Time>::CreateGate(stmt.gateType,
							gateQubits[0],
							nrQubits > 1 ? gateQubits[1] : 0,
							nrQubits > 2 ? gateQubits[2] : 0,
				...
				else
				{
					...
					if (stmt.paramsDecl.size() != stmt.parameters.size())
						throw std::invalid_argument("Uop operation: number of parameters do not match the declaration.");
					...
					if (stmt.qubits.size() % nrQubits != 0)
						throw std::invalid_argument("Defined Uop operation: number of qubits does not match the gate requirements.");
					...
					for (int pos = 0; pos < static_cast<int>(stmt.qubits.size()); pos += nrQubits)
					{
						...
						{
							StatementType gateStmt = addGate(op, qubitMap, opaqueGates, definedGates, variables);
			...
			{
				unsigned long long int condValue = static_cast<unsigned long long int>(stmt.condValue);
				...
				if (stmt.gateType != Circuits::QuantumGateType::kNone)
				{
					...
					const auto condition = Circuits::CircuitFactory<Time>::CreateEqualCondition(ind, condBits);
					...
					for (int pos = 0; pos < static_cast<int>(stmt.qubits.size()); pos += nrQubits)
					{
						...
						auto condOp = Circuits::CircuitFactory<Time>::CreateConditionalGate(gateOp, condition);
			...
			case QoperationStatement::OperationType::GateDecl: // do not generate anything here, it's already handled when the gate is called
			default:
				// those are ignored
				break;
			}
