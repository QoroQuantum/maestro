...
BOOST_FUSION_ADAPT_STRUCT(
	...
	(std::vector<qasm::StatementType>, statements)
)


namespace qasm {

	void printd(const double& v)
	{
		std::cout << "version: " << v << "\n";
	}
	...
	// 2. 'barrier' will be parsed, but ignored in the first phase. In this case we might want to add a 'barrier' operation in our circuit. For now it's not existent.
	// Adding it would have implications in circuit execution with the discrete event simulator and also in the transpiler functionality.

	template <typename Iterator = std::string::iterator, typename Skipper = ascii::space_type>
	struct QasmGrammar : qi::grammar<Iterator, Program(), Skipper>
	{
		QasmGrammar() : QasmGrammar::base_type{ program }
		{
			version = (qi::omit[qi::lexeme[qi::lit("OPENQASM") >> qi::space]] >> qi::double_ >> ';')[qi::_val = qi::_1];
			...
			statement = comment[qi::_val = AddComment(qi::_1)] |
				decl[qi::_val = AddDeclaration(qi::_1)] | 
				opaque[qi::_val = AddOpaqueDecl(qi::_1, std::ref(opaqueGates), std::ref(qreg_map))] |
				condOp[qi::_val = AddCondQop(qi::_1, std::ref(qreg_map), std::ref(creg_map), std::ref(opaqueGates), std::ref(definedGates))] |	
				gatedeclfull[qi::_val = AddGateDecl(qi::_1, std::ref(definedGates))] |
				qop[qi::_val = qi::_1];

			// this is the opaque gate declaration, it will be simply ignored (in 3.0 is supposed to be ignored)

			opaque %= qi::omit[qi::lexeme[qi::lit("opaque") >> qi::space]] >> identifier >> (('(' >> idList >> ')') | ('(' >> qi::eps >> ')') | qi::eps) >> idList >> ';';

			// **************************************************************************************************************************************************************

			// some of the more complex things

			gatedecl %= qi::omit[qi::lexeme[qi::lit("gate") >> qi::space]] >> identifier >> (('(' >> idList >> ')') | ('(' >> qi::eps >> ')') | qi::eps) >> idList >> '{';
			
			simplebarrier %= qi::omit[qi::lexeme[qi::lit("barrier") >> qi::space]] >> idList >> ';';
			...
			condOp %= qi::lit("if") >> '(' >> identifier >> qi::lit("==") >> qi::int_ >> ')' >> qop;

			simpleGatecall %= (identifier >> mixedList) | (identifier >> '(' >> ')' >> mixedList);
			expGatecall %= identifier >> '(' >> expList >> ')' >> mixedList;

			gatecall %= simpleGatecall | expGatecall;

			ugateCall %= (qi::lit("U") | qi::lit("u")) >> '(' >> expList >> ')' >> argument;
			cxgateCall %= qi::omit[qi::lexeme[(qi::lit("CX") | qi::lit("cx")) >> qi::space]] >> argument >> ',' >> argument;

			uop %= cxgateCall | ugateCall | gatecall;

			qop = (measureOp[qi::_val = AddMeasure(qi::_1, std::ref(creg_map), std::ref(qreg_map))] 
				| resetOp[qi::_val = AddReset(qi::_1, std::ref(qreg_map))] 
				| barrierOp[qi::_val = AddBarrier(qi::_1, std::ref(qreg_map))] 
				| uop[qi::_val = AddGate(qi::_1, std::ref(qreg_map), std::ref(opaqueGates), std::ref(definedGates))]) >> ';';

			// **************************************************************************************************************************************************************

			qregdecl %= (qi::omit[qi::lexeme[qi::lit("qreg") >> qi::space]] >> indexedId)[qi::_val = AddQreg(std::ref(qreg_counter), std::ref(qreg_map), qi::_1)] ;
			cregdecl %= (qi::omit[qi::lexeme[qi::lit("creg") >> qi::space]] >> indexedId)[qi::_val = AddCreg(std::ref(creg_counter), std::ref(creg_map), qi::_1)] ;

			decl %= (qregdecl | cregdecl) >> ';';

			measureOp %= qi::omit[qi::lexeme[qi::lit("measure") >> qi::space]] >> argument >> qi::lit("->") >> argument;
			resetOp %= qi::omit[qi::lexeme[qi::lit("reset") >> qi::space]] >> argument;
			barrierOp %= qi::omit[qi::lexeme[qi::lit("barrier") >> qi::space]] >> mixedList;

			// **************************************************************************************************************************************************************

			idList %= identifier % ',';
			
			indexedId = (identifier >> '[' >> qi::int_ >> ']')[qi::_val = MakeIndexedId(qi::_1, qi::_2)];
			...
			expression = (product >> qi::char_("+-") >> expression)[qi::_val = MakeBinary(qi::_2, qi::_1, qi::_3)] | product[qi::_val = qi::_1];
			product = (factor2 >> qi::char_("*/") >> product)[qi::_val = MakeBinary(qi::_2, qi::_1, qi::_3)] | factor2[qi::_val = qi::_1];

			factor2 = (factor >> '^' >> factor2)[qi::_val = MakeBinary('^', qi::_1, qi::_2)] | factor[qi::_val = qi::_1];
			unary = (qi::char_("+-") >> factor)[qi::_val = MakeUnary(qi::_1, qi::_2)];
			factor = group[qi::_val = qi::_1] | constant[qi::_val = qi::_1] | unary[qi::_val = qi::_1] | (funcName >> group)[qi::_val = MakeFunction(qi::_1, qi::_2)] | identifier[qi::_val = MakeVariable(qi::_1)];
			constant = qi::double_[qi::_val = MakeConstant(qi::_1)] | qi::int_[qi::_val = MakeConstant(qi::_1)] | pi[qi::_val = MakeConstant(qi::_1)];
			group %= '(' >> expression >> ')';

			funcName %= qi::string("sin") | qi::string("cos") | qi::string("tan") | qi::string("exp") | qi::string("ln") | qi::string("sqrt");
			...
			include %= qi::omit[qi::lexeme[qi::lit("include") >> qi::space]] >> quoted_string >> ';';
		...
		qi::rule<Iterator, boost::fusion::vector<GateDeclType, std::vector<GateDeclOpType>>(), Skipper> gatedeclfull;
		...
		qi::rule<Iterator, Expression(), Skipper> expression, group, product, factor, factor2;
